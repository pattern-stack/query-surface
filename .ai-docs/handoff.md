...
- **Prod catalog size** — a real org surfaces a very large catalog in `describe()`; accurate but large for an agent. Curation/allowlist is a future UX lever (the explicit `measureSpecs`/`dimensionSpecs` override path exists).
