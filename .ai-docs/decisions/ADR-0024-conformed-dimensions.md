...
## Amendment 4 — Conformed group dims resolve at EVERY measure leg source, not just the query root (2026-06-29, Dug)

The conformed-dimension rule (invariant #5) is enforced at the query **root**, but a multi-source `measure()` pre-aggregates each measure leg in its **own per-source CTE** (the fan-safe design — `multiSourceSelect`/`sourceSelect`). The per-source group-dim resolver (`lowerGroupDim` → `resolveJoinPlan`) only resolves a group dim that is **local**, **EAV-on-the-source**, or an explicitly **dotted** `entity.col` to-one path. A group dim given by its **bare root name** (e.g. `stage`, an EAV dim owned by `opportunities`) does **not** resolve for a non-root leg source: `resolveJoinPlan` treats an undotted name as `local` (`join-plan.ts:112-117`), so `colObj('observations','stage')` throws *"unknown column stage on observations"* (`compile-drizzle.ts:314` via `lowerGroupDim:732`).

**Surfaced** building the D2 derived cross-source fan-safety test (ADR-0029) — a derived metric `ExpectedRevenue.sum − count(observations)` grouped by `stage`. Grouping by `account_id` works (a **native** column physically on BOTH entities → each leg resolves it locally); grouping by `stage` fails (EAV on `opportunities` only → the `observations` leg needs the `observations→opportunities` to-one conformance). The plain (non-derived) ground-truth query fails identically — it is **not** a metric/derived bug; it is a hole in the conformed-dimension **execute** path for non-root sources.

**Should it work? Yes.** `observations belongs_to opportunities` is **to-one** (`schema.dealbrain:83`), so `opportunities.stage` is conformed at the observations grain by invariant #5 — the *exact* case the eav-to-one work already makes succeed for the **root** (`measure('observations', group_by:['opportunities.stage'])`). The only difference is that here observations is a measure **leg source**, not the query root.

**Decision (settled).** A group dim is legal at a multi-source aggregate iff it conforms — **local OR to-one chain (incl. EAV-on-to-one)** — at **every** measure leg source (the existing intersection rule, now enforced on the execute path, not just asserted). For each leg source, resolve the group dim to its **owner + column** (determined once from the root's conformed set) and lower via the **source→owner to-one path**. The **lowering already exists** — `lowerToOne` composes the native belongs_to chain AND the EAV value join on a to-one target (shipped by the eav-to-one work; `compile-drizzle.ts` lowerToOne). The gap is **purely resolution**: mapping a bare/root dim name through a non-root source's to-one path. Reuse `belongsToPaths` / `conformedDimensions` (`join-plan.ts`).

**Safety is unchanged** — a source reachable to the dim's owner only by a **to-many** path → **REJECT** (would fan); a **diamond** (≥2 distinct to-one paths) → **REJECT** (ambiguous). So this flips the genuinely-conformed (to-one) cases from a **false-refusal** into a **correct cross-grain result**, while keeping the unsafe directions refused. No new concept — it extends invariant #5's **reach** from the root to every leg source. Each leg still pre-aggregates 1:1 (1:1∘1:1), the metric/aggregate stays outer-SELECT, and the to-one `ON` folds target scope exactly as today (fail-closed). The pinned fail-loud boundary (derived-metric `D3c`) flips to a passing cross-grain test.

**Scope / non-goals.** Execute-path resolution ONLY — the conformed SET (`describe`/`conformedDimensions`) already advertises these dims. No change to the grain oracle, the fan-safety topology, or scope folding. Bare-name resolution gains a to-one target search (native + EAV); ambiguity (a bare name found on ≥2 to-one targets) rejects as a diamond, mirroring the dotted path.
