...
import type { DealbrainMeasureSpec } from '../../src/adapters/reference/model.dealbrain.ts';
...
type MeasureSpec = DealbrainMeasureSpec;
...
type BookMeasure = FieldMeasure | CountMeasure | RatioMeasure | DerivedMeasure | ExpressionMeasure;
...
function specsFromBook(book: BookMeasure[]): MeasureSpec[] {
  const byKey = new Map<string, MeasureSpec>(
    BASE_SPECS.map((s) => [s.key, { ...s, aggs: [...s.aggs] }]),
  );
  for (const m of book) {
...
async function resolvePks(): Promise<void> {
  ...
  try {
    for (const e of ENTITIES) PKS[e] = (await probe.service.describe(e)).primaryKey;
  } finally {
    await probe.close();
  }
}
...
function defsFromBook(book: BookMeasure[]): MeasureCatalog {
  const defs: MeasureCatalog = {};
  for (const m of book) {
    if (m.kind === 'field') {
      defs[m.slug] = {
        kind: 'atomic',
      ...
      defs[m.slug] = {
        kind: 'derived',
        expr: { op: m.op, left: { ref: m.left }, right },
        label: m.name,
      };
    }
  }
...
const MEASURE_PRESETS: Record<
  string,
  { title: string; blurb: string; entity: string; q: AggregateRequest }
> = {
  obs_by_stage: {
    title: 'Evidence by opportunity stage  ·  EAV dim through a to-one join',
    blurb:
      'Observations counted at THEIR grain, grouped by their opportunity’s EAV `stage` — reached through the belongs_to join (1:1 ∘ 1:1, grain-safe). The conformance just shipped.',
    entity: 'observations',
    q: {
      group_by: ['opportunities.stage'],
      measures: [{ on: '*', agg: 'count', as: 'observations' }],
      order_by: [{ on: 'observations', dir: 'desc' }],
    },
  },
  obs_by_band: {
    ...
    entity: 'observations',
    q: {
      group_by: ['opportunities.deal_size_band'],
      measures: [{ on: '*', agg: 'count', as: 'observations' }],
      order_by: [{ on: 'observations', dir: 'desc' }],
    },
  },
  pipeline_by_stage: {
    ...
    blurb:
      'At the opportunities grain: catalog measures referenced BY NAME (`{ref}`) — Σ ExpectedRevenue + count + avg win-prob — grouped by the EAV `stage` dimension. No inline {on,agg}.',
    entity: 'opportunities',
    q: {
      group_by: ['stage'],
      measures: [
  ...
  refused_to_many: {
    ...
    blurb:
      'Grouping opportunities by observations.type would FAN OUT the measure. The engine refuses — read the error; it’s the trust story.',
    entity: 'opportunities',
    q: {
...
const SELECT_PRESETS: Record<
  string,
  { title: string; blurb: string; entity: string; opts: QueryOptions }
> = {
  won_opps: {
    title: 'Opportunities in a won stage',
    blurb:
      'Structured filter on the EAV `StageName` field (the retrieval path addresses EAV fields by their field key, is_visible-gated); IDs + a preview, grain preserved.',
    entity: 'opportunities',
    opts: {
      filter: { on: 'StageName', op: 'eq', value: 'closed_won' },
      columns: ['id', 'StageName', 'Amount'],
      preview: true,
      page: { limit: 10 },
      include_sql: true,
    },
  ...
  relevant_obs: {
    ...
    blurb:
      'op:"relevant" with a top_k cutoff defines a cohort. Without OPENAI_API_KEY this is a verbatim phrase match (ILIKE stub); set the key for true concepts.',
    entity: 'observations',
    opts: {
      filter: { on: 'normalized_text', op: 'relevant', query: 'pricing', top_k: 10 },
      columns: ['type', 'normalized_text', 'opportunity_id'],
      preview: true,
      include_sql: true,
    },
  },
};
const COMPARE_PRESETS: Record<
  string,
  { title: string; blurb: string; entity: string; req: CompareRequest }
> = {
  won_vs_lost: {
    title: 'Won vs Lost pipeline  ·  variant-vs-variant',
    blurb:
      'One base metric (Σ ExpectedRevenue + deal count), two labeled variants overriding the stage filter, aligned. Each variant inherits fan-safety + fail-closed scope.',
    entity: 'opportunities',
    req: {
      measures: [
...
async function apiDescribe(entity: string) {
  ...
  const [catalog, conformed, measures] = await Promise.all([
    h.service.describe(entity),
    h.service.describeConformedDimensions(entity),
    h.service.describeMeasures(entity),
  ]);
...
async function apiMeasure(body: { preset?: string }) {
  ...
  const t0 = performance.now();
  const res = await h.service.measure(p.entity, p.q, {
    include_sql: true,
    citation: { boundary: true },
  });
  return {
    preset: { title: p.title, blurb: p.blurb, entity: p.entity, request: p.q },
    rows: res.rows,
    citation: res.citation,
    sql: pretty(res.sql),
    params: displayParams(res.params),
    ms: ms(t0),
  };
}
// The concept text of a top-level `relevant` leaf (labels the embedding param in the UI).
function relevantQuery(filter: FilterExpression | undefined): string | undefined {
  return filter && 'op' in filter && filter.op === 'relevant' ? filter.query : undefined;
}
async function apiSelect(body: { preset?: string }) {
  const p = SELECT_PRESETS[body.preset ?? 'won_opps'] ?? SELECT_PRESETS.won_opps;
  const t0 = performance.now();
  const res = await h.service.select(p.entity, p.opts);
  return {
    preset: { title: p.title, blurb: p.blurb, entity: p.entity, request: p.opts },
    total: res.total,
    rows: res.preview ?? res.ids?.map((id: string) => ({ id })) ?? [],
    ids: res.ids,
    sql: pretty(res.sql),
    params: displayParams(res.params, relevantQuery(p.opts.filter)),
    ms: ms(t0),
  };
}
async function apiFetch(body: { entity?: string; ids?: string[]; columns?: string[] }) {
  ...
  const t0 = performance.now();
  const res = await h.service.fetch(entity, ids, {
...
async function apiCompare(body: { preset?: string }) {
  ...
  const t0 = performance.now();
  const res = await h.service.compare(p.entity, p.req);
  return {
    preset: { title: p.title, blurb: p.blurb, entity: p.entity, request: p.req },
    result: res,
    ms: ms(t0),
  };
}

// A parsed JSON request body — every route coerces/validates the fields it reads.
type JsonBody = Record<string, unknown>;

// ── MEASURES workbench ─────────────────────────────────────────────────────────────────────────
// Resolve a book measure into the engine `measures[]` item (+ how it resolves, for the UI badge).
function resolveBookMeasure(m: BookMeasure): { item: MeasureRef; resolution: string } {
  ...
  return {
    item: { ref: m.slug },
    resolution: `engine catalog · {ref:"${m.slug}"}  (= ${formula})`,
  };
}

// The measure-eligible fields per entity (numeric fields → aggregatable). Drawn from describe().
async function measureEligibleFields(
  entity: string,
): Promise<{ key: string; type: string; label?: string }[]> {
  const cat = await h.service.describe(entity);
  return (cat.fields ?? [])
...
async function apiMeasuresInfo() {
  ...
  const [oppCat, obsCat, oppFields, metrics] = await Promise.all([
    h.service.describeMeasures('opportunities'),
    h.service.describeMeasures('observations'),
  ...
  const atomics = [...new Set([...oppCat, ...obsCat].map((m) => m.name))].sort();
  return {
...
async function apiMeasuresDefine(body: JsonBody): Promise<unknown> {
  ...
  if (BOOK.some((m) => m.slug === slug))
    throw new Error(`define: a measure with slug "${slug}" already exists`);
  ...
  if (body.kind === 'count') {
    ...
    const additivity = (body.additivity ??
      (agg === 'sum' || agg === 'count' ? 'additive' : 'non')) as FieldMeasure['additivity'];
    ...
    if (!numerator || !denominator)
      throw new Error(
        'define: a ratio needs a `numerator` and `denominator` (atomic measure slugs)',
      );
    if (numerator === denominator)
      throw new Error('define: a ratio numerator and denominator must differ');
    m = { slug, kind: 'ratio', name, numerator, denominator };
  } else if (body.kind === 'derived') {
    const op = String(body.op ?? '') as DerivedMeasure['op'];
    if (!['+', '-', '*', '/'].includes(op))
      throw new Error('define: a derived metric needs an `op` (one of + - * /)');
    ...
    if (!right)
      throw new Error(
        'define: a derived metric needs a `right` (atomic measure slug or numeric literal)',
      );
    if (rightIsLit && !Number.isFinite(Number(right)))
      throw new Error('define: a literal `right` must be a finite number');
    ...
    if (!['+', '-', '*', '/'].includes(op))
      throw new Error('define: an expression measure needs an `op` (one of + - * /)');
    const agg = String(body.agg ?? 'sum') as ExpressionMeasure['agg'];
    if (!['sum', 'avg', 'min', 'max'].includes(agg))
      throw new Error('define: an expression measure needs an `agg` (one of sum/avg/min/max)');
    const left = String(body.left ?? '');
    ...
    if (!right)
      throw new Error(
        'define: an expression measure needs a `right` (numeric field key or numeric literal)',
      );
    if (rightIsLit && !Number.isFinite(Number(right)))
      throw new Error('define: a literal `right` must be a finite number');
    // sum of a row-level product is host-declared additive (an additive amount × a ratio is summable).
    const additivity = (body.additivity ??
      (agg === 'sum' ? 'additive' : 'non')) as ExpressionMeasure['additivity'];
    m = {
      slug,
      kind: 'expression',
      name,
    ...
    throw new Error(
      `define: unknown kind "${body.kind}" (expected 'count' | 'field' | 'ratio' | 'derived' | 'expression')`,
    );
  }
  ...
  try {
    ...
    const t0 = performance.now();
    const res = await h.service.measure(entity, { measures: [item] }, { include_sql: true });
    saveBook(BOOK);
    // `item` is {ref:slug} — the probe proves the agent can call this measure by its name.
    return {
      ok: true,
      measure: { ...m, alias: aliasFor(m), resolution },
      probe: { value: res.rows?.[0], sql: pretty(res.sql), ms: ms(t0) },
    };
  } catch (e) {
    BOOK = prevBook;
    await rebuild();
...
async function apiMeasuresDelete(body: JsonBody): Promise<unknown> {
...
async function apiMeasuresTest(body: JsonBody): Promise<unknown> {
  const slug = String(body.slug ?? body.id ?? '');
  ...
  const q: AggregateRequest = {
    measures: [item],
    ...(dim ? { group_by: [dim], order_by: [{ on: aliasFor(m), dir: 'desc' }] } : {}),
  };
  const t0 = performance.now();
  const res = await h.service.measure(entity, q, { include_sql: true });
  return {
    measure: { ...m, alias: aliasFor(m), resolution },
    entity,
    request: q,
...
const POST_ROUTES: Record<string, (body: JsonBody) => Promise<unknown>> = {
...
const server = Bun.serve({
  ...
  async fetch(req) {
    ...
    if (route && req.method === 'POST') {
      try {
        return Response.json(await route((await req.json()) as JsonBody));
      } catch (e) {
